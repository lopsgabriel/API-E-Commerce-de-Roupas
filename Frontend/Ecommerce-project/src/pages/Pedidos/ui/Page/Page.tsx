                  ...
                  >
                            ...
                            {expandido != pedido.id &&(
                              <>
                                <div className="flex items-center justify-between ">
                                  <img src={produtos.find((produto) => produto.id === item.produto)?.imagem} className="w-20 h-20 my-2 mr-3 rounded-lg object-cover" />
                                </div>
                              </>
                            )}
                            {expandido === pedido.id && (
                              <div className=""> 
                                <a href={`produto/${item.produto}`}>
                                  <div className="hover:scale-105 duration-300 flex items-center">
                                    ...
                                    <div className="ml-2">
                                      <h1 className="text-sm text-grey-600 font-bold  max-w-[220px]">{produtos.find((produto) => produto.id === item.produto)?.nome}</h1>
                    ...
                    <div className="flex flex-col w-full">
                      <div className=" justify-start items-center">
                        <p className="text-sm font-light text-gray-500 pt-1">{pedido.itens.length} {pedido.itens.length === 1 ? "item" : "itens"}</p>
                      </div>
                      <div className="flex justify-between items-center w-full">
                        <div className="flex items-center justify-center">
                          <div className={`w-2 h-2 mr-1 rounded-full ${pedido.status === "pendente" ? "bg-yellow-500" : "bg-green-300"}`}></div>
                          <p className="text-sm font-light text-gray-600 pt-1">{pedido.status.charAt(0).toUpperCase() + pedido.status.slice(1).toLowerCase()}</p>
                        </div>
                        <div className="flex items-center justify-center">
